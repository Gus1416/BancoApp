...
import logicacreacional.ClienteCRUDSingleton;
import logicacreacional.CuentaCRUDSingleton;
import logicacreacional.OperacionCRUDSingleton;
import logicacreacional.OperacionSingleton;
...
public class Controller_CLI {
	...
	static ClienteCRUD CRUDCliente = ClienteCRUDSingleton.getInstance();
	static CuentaCRUD CRUDCuenta = CuentaCRUDSingleton.getInstance();
	static OperacionCRUD CRUDOperacion = OperacionCRUDSingleton.getInstance();
	static Operacion Operacion = OperacionSingleton.getInstance();
...
// VALIDACIONES / VERIFICACIONES ------------------------------------------------------------------------------------------
	...
	public static String validacionPIN(String pPIN) {
		if (validarPIN(pPIN) == true) {
			...
			System.out.println("debe incluir al menos una letra mayúscula, al menos un número y al menos un carácter especial)");
			return null;
		}
	}
...
// REGISTROS / CREACIONES --------------------------------------------------------------------------
	public static Cuenta crearCuentaCliente(Cliente clienteActual, String PinFinal, Double SaldoFinal) {
		...
		CRUDCuenta.registrarCuenta(cuentaTemp, clienteActual.getIdentificacion());
	...
	public static Cliente registrarCliente(String pIdentificacionFinal, String pPrimerApellidoFinal, String pSegundoApellidoFinal,
		...
		CRUDCliente.registrarCliente(nuevoCliente);
...
// LISTADOS ****************************************
	...
	public void listarClientesOrdenados() {
		ArrayList<Cliente> clientes = CRUDCliente.consultarClientes();
	...
	public void listarCuentasOrdenadas() {
		ArrayList<Cuenta> cuentas = CRUDCuenta.consultarCuentas();
		Cuenta[] orderCuentas;
	...
	public void listarCliente(ArrayList<Cliente> clientela, String identificadorConsulta) {
		for (int i = 0; i < clientela.size(); i++) {
			if (clientela.get(i).getIdentificacion().equals(identificadorConsulta)) {
				...
				ArrayList<Cuenta> cuentasClienteActual = CRUDCuenta.consultarCuentasCliente(identificadorConsulta);
	...
	public void listarDetalleCuenta(String numCuenta) {
		ArrayList<Cliente> clientela = CRUDCliente.consultarClientes();

		for (int i = 0; i < clientela.size(); i++) {
			Cliente ClienteTempo = clientela.get(i);
			ArrayList<Cuenta> accounts = CRUDCuenta.consultarCuentasCliente(clientela.get(i).getIdentificacion());
...
// OPERACIONES //////////////////////////////////////////////////////////////////////////////////////
	public int cambiarPIN(Cuenta cuentaActual, String pNuevoPin) {
		if (validacionPIN(pNuevoPin) != null) {
			...
			CRUDCuenta.cambiarPin(cuentaActual);
	...
	public void realizarDeposito(Cuenta cuentaActual, String montoDeposito) {
		...
		if ((deposito % 1) == 0) {
			...
			cantOPE = CRUDOperacion.obtenerCantidadOpeCuenta(cuentaActual.getNumeroCuenta());
			cuentaActual.depositarColones(deposito, cantOPE);

			CRUDCuenta.actualizarSaldo(cuentaActual);
			Operacion = cuentaActual.getOperaciones().get(cuentaActual.getOperaciones().size() - 1);
			CRUDOperacion.registrarOperacion(Operacion, cuentaActual.getNumeroCuenta());
...
// AUXILIARES ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	public static void registrarEnBitacora(String pTipoOperacion, String pVista) {
		...
		CRUDOperacion.registrarEnBitacora(registro1);
	}
...
// CONSULTAS ##########################################################################################
	public void consultaEstadoCuenta(Cuenta cuentaActual, int tipoConsulta) {
		...
		for (int i = 0; i < clientela.size(); i++) {
			Cliente ClienteTempo = clientela.get(i);
			ArrayList<Cuenta> cuentas = CRUDCuenta.consultarCuentasCliente(ClienteTempo.getIdentificacion());
	...
	public double consultaGananciasComision(int tipoConsulta) {
		...
		if (tipoConsulta == 0) {
			ArrayList<Cuenta> cuentasSistemas = CRUDCuenta.consultarCuentas();
			...
			if (validarCuenta(numCuenta) != null) {
				ArrayList<Cuenta> cuentaEspecifica = CRUDCuenta.consultarCuentas();
...
// SERVICIOS EXTERNOS----------------
	public void envioCorreo(String pCorreoReceptor, String pNombreCliente, String pNumCuenta) {
		ICorreo mail = new CorreoLenguajeDecorator(new Correo(pNombreCliente, pNumCuenta));
		mail.enviarCorreo(pCorreoReceptor);
	}
